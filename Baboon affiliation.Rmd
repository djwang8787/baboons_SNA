---
title: "Age, relative harem size and vigilance does not predict affiliative social networks between captive baboon one-male units"
author: "Laura Elizabeth Cheng Li Ling"
co-author: "Dajun Wang"
date: "1/12/2023"
...
pacman::p_load(igraph, igraphdata, tidyverse, qgraph, 
               kableExtra, influenceR, keyplayer, sna,
               gridExtra, ggplot2)
...
- Removed Ralph from the analyses because his activity / behaviours weren't recorded
- he is also a male without a harem, so it's not fitting for the study

```{r Baboon-network-edgelist}
...
```{r Baboon-network-adjacencymatrix}
...
```{r Baboon-network-ageasnodesize}
summary(Baboonaf)
V(Baboonaf)$name
...
```{r Degree, eval = F}
# Laura says strength is more useful because it will take account of "weights"
igraph::degree(Baboonaf, mode = "all") ##total affiliation interactions
igraph::degree(Baboonaf, mode = "out") # all outgoing affiliation interactions
igraph::degree(Baboonaf, mode = "in") # all incoming affiliation interactions; all based on counts

degreeaf <- igraph::degree(Baboonaf, mode = "all")

set.seed(1)
plot(Baboonaf, vertex.size=degreeaf*2, edge.arrow.size = 0.3, edge.width = E(Baboonaf)$weight/3, layout = layout.circle, vertex.label.cex = 0.6, vertex.label.dist = 2, edge.curved = 0.1) 
# Vertex size based on total degree; bigger means that they have more connections to other nodes
```
...
```{r Community-detection-undirected}
...
```{r Community-detection-directed}
plot(Baboonaf, #Baboonaf is a directed igraph object/class
     edge.arrow.size = 0.3, 
     edge.width = E(Baboonaf_ud)$weight/3, 
     layout = layout.circle, 
     vertex.label.cex = 0.6,
...
baboonaf_spinglass = cluster_spinglass(Baboonaf, weights = E(Baboonaf)$weight) #modularity 0.12
baboonaf_spinglass_membership <- data.frame(node = 1:gorder(Baboonaf), community = baboonaf_spinglass$membership)
table(baboonaf_spinglass_membership$community)
membership(baboonaf_spinglass)
modularity(baboonaf_spinglass) #0.12
V(Baboonaf)$community <- baboonaf_spinglass$membership # assign community membership as a node attribute
plot(Baboonaf, 
     vertex.color=V(Baboonaf)$community, 
     vertex.size = 10, 
     vertex.frame.color = 'black', 
     layout = layout_with_fr(Baboonaf), 
     vertex.label.cex = 1,
     vertex.label.dist = 2, 
     edge.width = E(Baboonaf)$weight/3)


Baboonaf_louvain <- cluster_louvain(Baboonaf_ud, weights = E(Baboonaf)$weight)
...
```{r Baboon-network-adjacencymatrix-aggression}
...
Does vigilance explain the aggression networks ?
...
```{r mantell tests}
ape::mantel.test(as.matrix(Baboonaf_am), 
                 as.matrix(Baboonagg_am), 
                 nperm = 999,
                 graph = T,
                 alternative = "two.sided",
                 main = "Mantel test on a 14 x 14 matrices comparing between the affiliative and agonistic interactions in Hamadryas baboons one-male units.",
xlab = "z-statistic", 
ylab = "Density",
...
# p value: < 0.05, 0.043
```
Mantell tests to examine for significant differences between matrices:
- 
